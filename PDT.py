...
for dataset in datasets.datasets:  
    for fold in range(0,10):
        trainX=pd.read_csv("data/folds_nodiscr/train/" +  dataset["filename"]+str(fold)+".csv",delimiter=",",index_col=False).drop(dataset["targetvariablename"],1)
        trainY=pd.read_csv("data/folds_nodiscr/train/" +  dataset["filename"]+str(fold)+".csv",delimiter=",",index_col=False, usecols=[dataset["targetvariablename"]])
        testX=pd.read_csv("data/folds_nodiscr/test/" +  dataset["filename"]+str(fold)+".csv",delimiter=",",index_col=False).drop(dataset["targetvariablename"],1)
